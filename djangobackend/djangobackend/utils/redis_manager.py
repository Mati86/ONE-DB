...
class RedisManager:
    ...
    def store_monitoring_data(self, device_id, component, parameter, value, timestamp=None):
        ...
        # Try to convert to float, otherwise store as string, handle None
        try:
            if value is None:
                store_value = ""
            else:
        ...
        try:
            ...
            self.redis_client.zremrangebyrank(timeseries_key, 0, -1001)  # Keep last 1000 entries

            logger.debug(f"Stored monitoring data: {key} = {store_value}")
            return True
        except Exception as e:
            logger.error(
                f"Failed to store monitoring data: {e} | device_id={device_id}, "
                f"component={component}, parameter={parameter}, value={value}, "
                f"store_value={store_value}, data={data}"
            )
            return False
